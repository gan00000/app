...
//	protected ProgressBar pb;
	...
	public void init(Bundle bundle) {
		if (bundle != null) {
			...
			if(mComeFrom == Web.WEB_GO_WITH_URL){
				curUrl = (String) bundle.getSerializable(Const.BEAN_KEY);
			}else if (bundle.getSerializable(Const.BEAN_KEY) != null) {
		...
		progressDialog = (CustomProgressDialog) findViewById(E_id.js_custom_progress);
		// 获取系统的TelephonyManager对象
...
//		mWebView.addJavascriptInterface(openForJava(), JAVASCRIPTINTERFACE_KEFU);
...
//			break;
		case Web.WEB_GO_WELFARE_LIST:
			titleView.setTitleLeftStatus(View.GONE);
			titleView.setTitleRightTextRes(E_string.efun_pd_close);
			titleView.setTitleRightTVRes(E_drawable.efun_pd_cons_00a0e9_bg);
			titleView.setTitleRightTextColor(E_color.e_00a0e9);
			titleView.setTitleCenterText(((WelfareItemBean) mBundle.getSerializable(Const.BEAN_KEY)).getTitle());
			break;
		case Web.WEB_GO_CONNECTED_POST_TIPS:
			titleView.setTitleCenterText("发帖");
			titleView.setTitleRightStatus(View.GONE);
			break;
		case Web.WEB_GO_WITH_URL:
			titleView.setTitleRightTextRes(E_string.efun_pd_close);
			titleView.setTitleRightTVRes(E_drawable.efun_pd_cons_00a0e9_bg);
			titleView.setTitleRightTextColor(E_color.e_00a0e9);
			titleView.setTitleLeftStatus(View.GONE);
	...
	public void onClickRight() {
		...
		case Web.WEB_GO_VIP://VIP
			...
			mWebView = null;
			break;
		case Web.WEB_GO_WELFARE_LIST://福利列表
			finish();
			mWebView = null;
			break;
		case Web.WEB_GO_CONNECTED_POST_TIPS:
...
//			mWebView.loadUrl(createScatchUrl());
		...
		case Web.WEB_GO_VIP://VIP
			...
			break;
		case Web.WEB_GO_WELFARE_LIST:
			url = ((WelfareItemBean) mBundle
					.getSerializable(Const.BEAN_KEY)).getUrl();
			mWebView.loadUrl(((WelfareItemBean) mBundle
					.getSerializable(Const.BEAN_KEY)).getUrl());
//			mWebView.loadUrl("http://m.efunen.com/testTools.html");
			break;
		case Web.WEB_GO_CONNECTED_POST_TIPS:
			ConfigInfoBean bean = ProcessDatasUtils.getPostTipsInfo();
			if(bean != null){
	...
	private void sendMessage(String phone, String message) {
		...
		protected void onActivityResult(int requestCode, int resultCode, Intent data) {
			...
			if(mJSUsedObject != null){
				((PlatformNative2JS) mJSUsedObject).onActivityResult(requestCode, resultCode, data);
			}
		...
		private Native2JS openForJava() {
			// TODO Auto-generated method stub
			mJSUsedObject = new PlatformNative2JS(this, this, mWebView);
