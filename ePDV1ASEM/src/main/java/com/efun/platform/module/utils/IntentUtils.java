...
public class IntentUtils {
	 ...
	 * @param value
	 */
	public static void go2WithJSWeb(Context context , String url) {
		Intent it = new Intent(context, WebWithJsActivity.class);
		Bundle bundle = new Bundle();
		bundle.putInt(Web.WEB_GO_KEY, Web.WEB_GO_WITH_URL);
		if(url!=null){
			bundle.putSerializable(Const.BEAN_KEY, url);
		}
		it.putExtra(Const.DATA_KEY, bundle);
		context.startActivity(it);
	}
	...
	/**
	 * 跳转SummaryListActivity页面
	 * 
	 * @param context
	 * @param value
	 */
	public static void go2SummaryList(Context context, int value) {
		Intent it = new Intent(context, SummaryListActivity.class);
		Bundle bundle = new Bundle();
