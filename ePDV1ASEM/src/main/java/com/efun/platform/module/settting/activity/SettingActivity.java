...
public class SettingActivity extends FixedActivity{
	...
	private LinearLayout mCheckUpdateLayout,mPushFriendsLayout,mhelpOrQSLayout,mScorePDLayout,mLoginOut,mClearDataLayout;
	...
	public void init(Bundle bundle) {
		...
		mClearDataLayout = (LinearLayout) findViewById(E_id.contaier_linear_setting_clear_data);
		mLoginOut = (LinearLayout) findViewById(E_id.login_out);
...
//				TrackingUtils.track(mContext,TrackingUtils.ACTION_SETTING, TrackingUtils.NAME_SETTING_SCORE_APP);
		...
		mClearDataLayout.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				try {
					EfunFileUtil.deleteFile(getApplicationContext().getCacheDir());
					EfunFileUtil.deleteFile(getApplicationContext().getFilesDir());
					ToastUtils.toast(getApplicationContext(),"已清除");
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
