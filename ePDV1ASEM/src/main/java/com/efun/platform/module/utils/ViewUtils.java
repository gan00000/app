...
//	}
	 ...
	 * @return
	 */
	public static Dialog createToastChangeNickName(Context context, String titleStr,
			final OnEfunEditTextItemClickListener onEfunItemClickListener) {
		LayoutInflater inflater = LayoutInflater.from(context);
		final View v = inflater.inflate(E_layout.efun_pd_toast_common_edittext, null);
		final EditText msgTv = (EditText) v.findViewById(E_id.edit_1);
		TextView okBtn = (TextView) v.findViewById(E_id.confirm);
		TextView cancelBtn = (TextView) v.findViewById(E_id.cancle);
		final Dialog loadingDialog = new Dialog(context, E_style.Transparent);
		loadingDialog.setContentView(v, new LinearLayout.LayoutParams(
		...
		okBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (onEfunItemClickListener != null) {
					onEfunItemClickListener.onItemClick(0, msgTv.getText().toString());
					loadingDialog.dismiss();
				}
			}
		});
		cancelBtn.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				if (onEfunItemClickListener != null) {
					onEfunItemClickListener.onItemClick(1, msgTv.getText().toString());
					loadingDialog.dismiss();
				}
			}
		});
