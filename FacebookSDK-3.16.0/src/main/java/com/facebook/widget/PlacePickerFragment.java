...
public class PlacePickerFragment extends PickerFragment<GraphPlace> {
    ...
    public PlacePickerFragment(Bundle args) {
        super(GraphPlace.class, "com_facebook_placepickerfragment", args);
    ...
    void setupViews(ViewGroup view) {
        if (showSearchBox) {
            ListView listView = (ListView) view.findViewById(ResHelper.findViewIdByName(getActivity(), "com_facebook_picker_list_view"));

            View searchHeaderView = getActivity().getLayoutInflater().inflate(
                    ResHelper.findLayoutIdByName(getActivity(), "com_facebook_picker_search_box"), listView, false);

            listView.addHeaderView(searchHeaderView, null, false);

            searchBox = (EditText) view.findViewById(ResHelper.findViewIdByName(getActivity(), "com_facebook_picker_search_text"));
    ...
    String getDefaultTitleText() {
        return ResHelper.findStringByName(getActivity(), "com_facebook_nearby");
    }
        ...
        PickerFragmentAdapter<GraphPlace> adapter = new PickerFragmentAdapter<GraphPlace>(
            ...
            protected CharSequence getSubTitleOfGraphObject(GraphPlace graphObject) {
                ...
                if (category != null && wereHereCount != null) {
                    result = getString(ResHelper.findStringIdByName(getActivity(), "com_facebook_placepicker_subtitle_format"), category, wereHereCount);
                } else if (category == null && wereHereCount != null) {
                    result = getString(ResHelper.findStringIdByName(getActivity(), "com_facebook_placepicker_subtitle_were_here_only_format"), wereHereCount);
                } else if (category != null && wereHereCount == null) {
                    result = getString(ResHelper.findStringIdByName(getActivity(), "com_facebook_placepicker_subtitle_catetory_only_format"), category);
                }
                return result;
            }
            ...
            protected int getGraphObjectRowLayoutId(GraphPlace graphObject) {
                return ResHelper.findLayoutIdByName(getActivity(), "com_facebook_placepickerfragment_list_row");
            }

            @Override
            protected int getDefaultPicture() {
                return ResHelper.findDrawableIdByName(getActivity(), "com_facebook_place_default_icon");
            }
